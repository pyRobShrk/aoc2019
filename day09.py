...
class intCode:
    ...
    def run(self,inputs=[]):
        ...
        while self.code[i] != 99:
            opcode = self.code[i]
            p1, p2, p3 = self.code[i+1:i+4]
            
            #parameter three mode
            if opcode > 20000: # only relative mode supported
                opcode -= 20000
                p3 = self.relBase + p3
            
            # parameter two mode
            if opcode > 1000: # immediate mode
                opcode -= 1000
                if opcode > 1000: # relative mode
                    opcode -= 1000
                    p2 = self.code[self.relBase+p2]
            elif str(opcode)[-1] not in '349': # position mode
                p2 = self.code[p2]

            # parameter one mode
            if opcode > 100: # immediate mode
                opcode -= 100
                if opcode > 100: # relative mode
                    opcode -= 100
                    p1 = self.code[self.relBase+p1] if opcode != 3 else self.relBase+p1
            elif opcode != 3: # position mode
                p1 = self.code[p1]
            ...
            if opcode == 1:
                ...
                self.i = i + 2 # save state for restart
                return p1
            elif opcode == 5:
                ...
                self.relBase += p1
            i += 4 if opcode in (1,2,7,8) else 2 if opcode in (3,9) else 0
        self.halt = True
