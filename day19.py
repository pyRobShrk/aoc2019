...
for r in range(50):
    ...
    for c in range(50):
        drone = intCode(DRONE)
        row.append( drone.run([r,c]) )
    tabl.append(row)
beam = np.array(tabl)
print (beam.sum())

def testcorners(squareSize,row,col):
    drone = intCode(DRONE)
    ll = bool(drone.run([row+squareSize,col]))
    drone = intCode(DRONE)
    ur = bool(drone.run([row,col+squareSize]))
    return ll and ur

# location of upper left corner of 1x1, 2x2, 3x3, 4x4 follow a pattern
# continuing that pattern gets me close enough to test lots of upper left corners
# don't need to check the whole square just the upper right and lower left corners
tabl = []
for r in range(800,819):
    row = []
    for c in range(1000,1015):
        row.append( testcorners(99,r,c) )
    tabl.append(row)
    
# found first 100x100 square at x=807, y=1006
